...
# Forecast settings
FORECAST_HORIZON_SEC = 1800
TELEMETRY_SAMPLE_SEC = 1.0
...
PREDICTOR_MODE = os.getenv("LONG_TERM_LSTM_PREDICTOR_MODE", "OFFLINE").strip().upper()
...
# Sequence + feature settings (same core features as long_term_predictor.py)
SEQUENCE_LENGTH = 120
LAG_STEPS = [1, 5, 15, 60, 300]
...
# Rolling raw buffers
power_buffer = deque(maxlen=BUFFER_MAXLEN)
temp_buffer = deque(maxlen=BUFFER_MAXLEN)
humidity_buffer = deque(maxlen=BUFFER_MAXLEN)
time_buffer = deque(maxlen=BUFFER_MAXLEN)
...
# Step-wise feature history (each item is feature vector at one time step)
step_feature_buffer = deque(maxlen=BUFFER_MAXLEN)

# Pending supervised samples: (target_index, sequence_features)
pending_supervised = deque()
sample_index = 0

# Training datasets
train_X = deque(maxlen=5000)  # each item: (seq_len, feature_dim)
train_y = deque(maxlen=5000)  # each item: scalar future power

# Evaluation tracking (for EVAL_ONLY)
pending_forecasts = deque(maxlen=5000)  # each: (target_ts, predicted_power_w)
eval_errors = deque(maxlen=10000)
...
LSTM_MODEL_PATH = os.path.join(MODEL_DIR, f"{PLANT_IDENTIFIER}_long_lstm.pt")
LSTM_X_SCALER_PATH = os.path.join(MODEL_DIR, f"{PLANT_IDENTIFIER}_long_lstm_x_scaler.pkl")
LSTM_Y_SCALER_PATH = os.path.join(MODEL_DIR, f"{PLANT_IDENTIFIER}_long_lstm_y_scaler.pkl")
LSTM_META_PATH = os.path.join(MODEL_DIR, f"{PLANT_IDENTIFIER}_long_lstm.meta.json")
...
def reset_runtime_buffers():
    ...
    time_buffer.clear()
    step_feature_buffer.clear()
    pending_supervised.clear()
    train_X.clear()
    train_y.clear()
    sample_index = 0
    process_elapsed_sec = 0.0
...
def load_model_state():
    global model, model_ready, model_feature_dim
    try:
        required = [LSTM_MODEL_PATH, LSTM_X_SCALER_PATH, LSTM_Y_SCALER_PATH, LSTM_META_PATH]
        if any(not os.path.exists(p) for p in required):
            return False
        with open(LSTM_META_PATH, "r", encoding="utf-8") as f:
            saved_meta = json.load(f)
        if saved_meta != _model_meta():
            print("[predictor-long-lstm] saved model meta mismatch; ignoring saved model")
            return False
        if torch is None:
            return False
        package = torch.load(LSTM_MODEL_PATH, map_location="cpu")
        feature_dim = int(package.get("feature_dim", 0))
        if feature_dim <= 0:
            print("[predictor-long-lstm] invalid saved feature_dim; ignoring saved model")
            return False
        model_loaded = build_lstm_model(feature_dim)
        if model_loaded is None:
            return False
        model_loaded.load_state_dict(package["state_dict"])
        model_loaded.eval()
        with open(LSTM_X_SCALER_PATH, "rb") as f:
            x_scaler_loaded = pickle.load(f)
        with open(LSTM_Y_SCALER_PATH, "rb") as f:
            y_scaler_loaded = pickle.load(f)
        model = model_loaded
        model_feature_dim = feature_dim
        x_scaler.__dict__.update(x_scaler_loaded.__dict__)
        y_scaler.__dict__.update(y_scaler_loaded.__dict__)
        model_ready = True
        print(f"[predictor-long-lstm] loaded saved model: {LSTM_MODEL_PATH}")
        return True
    except Exception as e:
        print(f"[predictor-long-lstm] failed to load saved model: {e}")
        return False
        ...
        torch.save(
            {
                ...
                "feature_dim": int(model_feature_dim),
            },
            LSTM_MODEL_PATH,
        )
        with open(LSTM_X_SCALER_PATH, "wb") as f:
            pickle.dump(x_scaler, f)
        with open(LSTM_Y_SCALER_PATH, "wb") as f:
            pickle.dump(y_scaler, f)
        with open(LSTM_META_PATH, "w", encoding="utf-8") as f:
            json.dump(_model_meta(), f, indent=2)
    except Exception as e:
        print(f"[predictor-long-lstm] failed to save model: {e}")
...
def _update_eval_errors(sample_ts: float, actual_power_w: float) -> None:
    while pending_forecasts and pending_forecasts[0][0] <= sample_ts:
        _, pred_power = pending_forecasts.popleft()
        eval_errors.append(pred_power - actual_power_w)
...
def _eval_metrics() -> Optional[Dict[str, float]]:
    ...
    return {"mae_w": round(mae, 4), "rmse_w": round(rmse, 4), "count": int(len(eval_errors))}
...
def _update_load_aggregates(loads: Dict[str, Any]) -> None:
    ...
    for _, ld in loads.items():
        ...
        if on:
            ...
            duty_sum_on += duty
            try:
                prio = float(ld.get("priority", 0.0))
            except Exception:
                prio = 0.0
            priority_sum_on += prio
            priority_min_on = min(priority_min_on, prio)
            priority_max_on = max(priority_max_on, prio)
    ...
    load_priority_avg_on = float(priority_sum_on / on_count) if on_count > 0 else 0.0
    load_priority_min_on = float(priority_min_on) if on_count > 0 else 0.0
    load_priority_max_on = float(priority_max_on) if on_count > 0 else 0.0
...
def _handle_eval_only(client, sample_ts: float, process: Dict[str, Any], process_enabled: bool) -> None:
    ...
    if eval_last_process_enabled and not process_enabled:
        ...
        if metrics:
            summary_payload = {
                "schema_version": "1.1",
                "producer": "long_term_predictor_lstm",
            ...
            client.publish(TOPIC_PREDICTION_LONG_LSTM_NAME, json.dumps(summary_payload), qos=0)
...
def build_step_feature():
    """Build one feature vector using the same feature family as RandomForest predictor."""
    if not power_buffer:
        return None
    ...
    for lag in LAG_STEPS:
        if len(p_arr) > lag:
            features.append(float(p_arr[-lag]))
        else:
            features.append(float(p_arr[0]))
    ...
    for w in ROLLING_WINDOWS:
        if len(p_arr) >= w:
            ...
            features.append(float(p_arr.mean()))
            features.append(float(p_arr.std()))
    ...
    # system settings
    features.append(float(system_max_total_power_w))
    features.append(float(system_energy_goal_wh))
    features.append(float(system_control_policy_code))
    features.append(float(system_manual_override_flag))
    features.append(float(system_tariff_offpeak_flag))
    features.append(float(system_tariff_midpeak_flag))
    features.append(float(system_tariff_peak_flag))
    features.append(float(system_tariff_rate_per_kwh))
    features.append(float(system_peak_event_flag))
    ...
    features.append(float(system_minute_of_day_norm))
    features.append(float(system_minutes_to_peak_norm))
    features.append(float(system_minutes_to_offpeak_norm))

    # process timing
    features.append(float(process_elapsed_sec))
    features.append(float(process_last_duration_sec))
    features.append(float(process_avg_duration_sec))
    features.append(float(process_time_since_end_sec))

    # process targets (tank goals)
    features.append(float(process_target_tank1_level_pct))
    features.append(float(process_target_tank2_level_pct))
    features.append(float(process_target_tank1_temp_c))
    features.append(float(process_target_tank2_temp_c))
    features.append(float(process_goal_cycles))
    features.append(float(process_tank1_height_cm))
    features.append(float(process_tank2_height_cm))
    features.append(float(process_tank1_low_level_pct))
    features.append(float(process_tank2_low_level_pct))
    features.append(float(process_tank1_high_level_pct))
    features.append(float(process_tank2_high_level_pct))
    ...
    features.append(float(load_total_count))
    features.append(float(load_on_count))
    features.append(float(load_on_ratio))
    features.append(float(load_duty_sum_all))
    features.append(float(load_duty_sum_on))
    features.append(float(load_duty_avg_all))
    features.append(float(load_duty_avg_on))
    features.append(float(load_priority_avg_on))
    features.append(float(load_priority_min_on))
    features.append(float(load_priority_max_on))
...
def _step_feature_with_load_overrides(load_metrics: Dict[str, float], power_override: Optional[float] = None):
    if not power_buffer:
        return None
    p_arr = np.array(power_buffer, dtype=float)
    ...
    # aggregate load features (override)
    features.append(float(load_metrics.get("load_total_count", load_total_count)))
    features.append(float(load_metrics.get("load_on_count", load_on_count)))
    features.append(float(load_metrics.get("load_on_ratio", load_on_ratio)))
    features.append(float(load_metrics.get("load_duty_sum_all", load_duty_sum_all)))
    features.append(float(load_metrics.get("load_duty_sum_on", load_duty_sum_on)))
    features.append(float(load_metrics.get("load_duty_avg_all", load_duty_avg_all)))
    features.append(float(load_metrics.get("load_duty_avg_on", load_duty_avg_on)))
    features.append(float(load_metrics.get("load_priority_avg_on", load_priority_avg_on)))
    features.append(float(load_metrics.get("load_priority_min_on", load_priority_min_on)))
    features.append(float(load_metrics.get("load_priority_max_on", load_priority_max_on)))
...
def build_mpc_process_plan_suggestions(
    ...
    if not MPC_PROCESS_PLANNER_ENABLED or not MPC_ENABLED or not model_ready or model is None:
        return []
    if not bool(process.get("enabled", False)):
        return []
    if len(step_feature_buffer) < SEQUENCE_LENGTH:
        return []
    ...
    for mode, dev in candidates:
        ...
        for duty in duty_steps:
            ...
            step_feat = _step_feature_with_load_overrides(load_metrics, power_override=total_power_candidate)
            if step_feat is None:
                continue
            seq = np.array(list(step_feature_buffer)[-SEQUENCE_LENGTH:], dtype=float)
            seq[-1] = np.array(step_feat, dtype=float)
            x_now = np.array([seq], dtype=float)
            x_now_s = _scale_X(x_now)
            y_hat_s = _predict_scaled_batch(x_now_s)
            predicted_power = float(y_scaler.inverse_transform(y_hat_s.reshape(-1, 1))[0][0])
...
def build_mpc_suggestions(
    ...
    if not MPC_ENABLED or not model_ready or model is None:
        return []
    if not bool(process.get("enabled", False)):
        return []
    if len(step_feature_buffer) < SEQUENCE_LENGTH:
...
def build_long_term_suggestions(
    ...
    predicted_incremental_energy_wh: float,
    ...
    peak_risk = (predicted_power_w > (max_total_power_w * RISK_MARGIN_RATIO)) or (total_power_w > max_total_power_w)
    projected_energy_1d_wh = current_energy_1d_wh + predicted_incremental_energy_wh
    energy_budget_risk = (max_energy_1d_wh > 0.0) and (projected_energy_1d_wh >= max_energy_1d_wh * RISK_MARGIN_RATIO)

    if (max_energy_1d_wh > 0.0 and projected_energy_1d_wh > max_energy_1d_wh * 1.01) or predicted_power_w > max_total_power_w * 1.05:
        risk_level = "HIGH"
    ...
    if not (peak_risk or energy_budget_risk):
        return [], "NONE", risk_level, False

    reason = "ENERGY_BUDGET" if energy_budget_risk and not peak_risk else "LONG_TERM_RISK"
    ...
    return suggestions, reason, risk_level, (peak_risk or energy_budget_risk)
...
def build_lstm_model(feature_dim: int):
    if torch is None:
        return None
    net = PowerLSTM(feature_dim)
    net.train()
    return net


def _predict_scaled_batch(Xs: np.ndarray) -> np.ndarray:
    if model is None or torch is None:
    ...
    with torch.no_grad():
        ...
        y_t = model(x_t)
    return y_t.detach().cpu().numpy().reshape(-1, 1)


def _fit_scaled_batch(Xs: np.ndarray, ys: np.ndarray):
    if model is None or torch is None:
        raise RuntimeError("model backend unavailable")
...
def _scale_X(X):
    """Scale 3D sequence tensor with a feature scaler fitted on flattened features."""
    n, t, f = X.shape
    X2 = X.reshape(n * t, f)
    X2s = x_scaler.transform(X2)
    return X2s.reshape(n, t, f)


def apply_config(cfg: dict):
    global FORECAST_HORIZON_SEC, TELEMETRY_SAMPLE_SEC, FORECAST_HORIZON_STEPS
    global TRAIN_INTERVAL_SEC, TRAIN_INTERVAL_STEPS, MIN_TRAIN_SAMPLE_COUNT
    global RISK_MARGIN_RATIO, MAX_SUGGESTIONS, MAX_ENERGY_CONSUMPTION_FOR_THR_DAY_WH
    global SEQUENCE_LENGTH, EPOCHS, BATCH_SIZE, LEARNING_RATE
    global model, model_ready, model_feature_dim, PREDICTOR_MODE, warned_eval_without_model
    ...
    if "SEQUENCE_LENGTH" in cfg:
        SEQUENCE_LENGTH = max(10, int(cfg["SEQUENCE_LENGTH"]))
    if "EPOCHS" in cfg:
    ...
    # Rebuild model if architecture-sensitive config changed.
    model = None
    model_ready = False
    model_feature_dim = 0

    print("[predictor-long-lstm] config updated:")
    print(f"  - mode={PREDICTOR_MODE} train_enabled={_training_enabled()} predict_enabled={_prediction_enabled()}")
    print(f"  - horizon_sec={FORECAST_HORIZON_SEC} sample_sec={TELEMETRY_SAMPLE_SEC} seq_len={SEQUENCE_LENGTH}")
...
def on_connect(client, userdata, flags, reason_code, properties):
    if reason_code == 0:
        print(
            f"[predictor-long-lstm] connected: telemetry={TOPIC_TELEMETRY_NAME} "
            f"cmd={TOPIC_PREDICTOR_LONG_LSTM_CMD_NAME}"
        )
        client.subscribe(TOPIC_TELEMETRY_NAME)
        client.subscribe(TOPIC_PREDICTOR_LONG_LSTM_CMD_NAME)
    else:
        print(f"[predictor-long-lstm] connect failed: reason_code={reason_code}")


def on_message(client, userdata, msg):
    global model_ready, model, model_feature_dim, sample_index, last_train_index, warned_eval_without_model, eval_last_process_enabled
    global process_target_tank1_level_pct, process_target_tank2_level_pct
    global process_target_tank1_temp_c, process_target_tank2_temp_c, process_goal_cycles
    global process_cycle_count, process_progress_ratio, process_state_code
    global process_tank1_level_pct, process_tank2_level_pct
    ...
    if msg.topic == TOPIC_PREDICTOR_LONG_LSTM_CMD_NAME:
        apply_config(data)
        return
    ...
    _update_eval_errors(sample_ts, total_power_w)
    ...
    step_features = build_step_feature()
    if step_features is None:
        return

    step_feature_buffer.append(step_features)

    if _training_enabled():
        # Create future-supervised sample request once sequence is available.
        if len(step_feature_buffer) >= SEQUENCE_LENGTH:
            seq = np.array(list(step_feature_buffer)[-SEQUENCE_LENGTH:], dtype=float)
            target_idx = sample_index + FORECAST_HORIZON_STEPS
            pending_supervised.append((target_idx, seq))

        # Resolve any due targets with current observed power.
        while pending_supervised and pending_supervised[0][0] <= sample_index:
            _, seq_X = pending_supervised.popleft()
            train_X.append(seq_X)
            train_y.append(total_power_w)

        # Periodic batch retraining.
        if len(train_X) >= MIN_TRAIN_SAMPLE_COUNT and (sample_index - last_train_index) >= TRAIN_INTERVAL_STEPS:
            X = np.array(train_X, dtype=float)
            y = np.array(train_y, dtype=float).reshape(-1, 1)

            # Fit scalers on available training data.
            ...
            if model is None or model_feature_dim != feature_dim:
                model = build_lstm_model(feature_dim=feature_dim)
                model_feature_dim = feature_dim
            _fit_scaled_batch(Xs, ys)
            model_ready = True
            warned_eval_without_model = False
            last_train_index = sample_index
            print(f"[predictor-long-lstm] trained model on {len(X)} samples")
            save_model_state()
    ...
    if not model_ready:
        if not warned_eval_without_model:
            print("[predictor-long-lstm] mode requires prediction, but model is not ready. Train first or load saved weights.")
            warned_eval_without_model = True
        return
    if len(step_feature_buffer) < SEQUENCE_LENGTH:
        return

    # Predict horizon power.
    x_now = np.array([list(step_feature_buffer)[-SEQUENCE_LENGTH:]], dtype=float)
    x_now_s = _scale_X(x_now)
    y_hat_s = _predict_scaled_batch(x_now_s)
    predicted_power_w = float(y_scaler.inverse_transform(y_hat_s.reshape(-1, 1))[0][0])
    predicted_incremental_energy_wh = max(0.0, predicted_power_w) * (FORECAST_HORIZON_SEC / 3600.0)
    predicted_total_energy_wh = current_total_energy_wh + predicted_incremental_energy_wh
    projected_energy_window_wh = current_energy_window_wh + predicted_incremental_energy_wh
    ...
    suggestions, suggestion_reason, risk_level, peak_risk = build_long_term_suggestions(
        loads,
        total_power_w,
        predicted_power_w,
        max_total_power_w,
        current_energy_window_wh,
        predicted_incremental_energy_wh,
    ...
    energy_budget_risk = (max_energy_window_wh > 0.0) and (projected_energy_window_wh >= max_energy_window_wh * RISK_MARGIN_RATIO)
    if PREDICTOR_MODE == "EVAL_ONLY":
        pending_forecasts.append((sample_ts + FORECAST_HORIZON_SEC, predicted_power_w))
    ...
    client.publish(TOPIC_PREDICTION_LONG_LSTM_NAME, json.dumps(payload), qos=0)


def main():
    global PREDICTOR_MODE
    PREDICTOR_MODE = _normalize_predictor_mode(PREDICTOR_MODE)
    if torch is None:
        print("[predictor-long-lstm] PyTorch not installed.")
        print("[predictor-long-lstm] Install with: pip install torch")
        return

    print("[predictor-long-lstm] starting LSTM long-term predictor (PyTorch backend)...")
    print(f"[predictor-long-lstm] mode={PREDICTOR_MODE} train_enabled={_training_enabled()} predict_enabled={_prediction_enabled()}")
    load_model_state()
    client = mqtt.Client(
        client_id=f"predictor-long-lstm-{PLANT_IDENTIFIER}-{int(time.time())}",
    ...
    if MQTT_USERNAME and MQTT_PASSWORD:
        ...
        print("[predictor-long-lstm] warning: HIVEMQ_USER/HIVEMQ_PASS not set")
