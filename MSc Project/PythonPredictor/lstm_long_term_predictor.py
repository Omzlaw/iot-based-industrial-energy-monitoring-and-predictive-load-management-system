...
# Forecast settings
FORECAST_HORIZON_SEC = _env_int("LONG_TERM_LSTM_FORECAST_HORIZON_SEC", 1800)
TELEMETRY_SAMPLE_SEC = _env_float("LONG_TERM_LSTM_TELEMETRY_SAMPLE_SEC", 1.0)
FORECAST_HORIZON_STEPS = int(FORECAST_HORIZON_SEC / TELEMETRY_SAMPLE_SEC)
TRAIN_INTERVAL_SEC = _env_int("LONG_TERM_LSTM_TRAIN_INTERVAL_SEC", 60)
TRAIN_INTERVAL_STEPS = int(TRAIN_INTERVAL_SEC / TELEMETRY_SAMPLE_SEC)
MIN_TRAIN_SAMPLE_COUNT = _env_int("LONG_TERM_LSTM_MIN_TRAIN_SAMPLE_COUNT", 300)
...
PREDICTOR_MODE = os.getenv("LONG_TERM_LSTM_PREDICTOR_MODE", "ONLINE").strip().upper()
...
LSTM_ANALYTICS_PATH = os.path.join(MODEL_DIR, f"{PLANT_IDENTIFIER}_long_lstm.analytics.json")
...
def _model_meta_compat(saved_meta: dict) -> tuple[bool, list[str], dict]:
    ...
    hard_keys = ("model_type", "target_type", "plant_id", "sequence_length", "lag_steps", "rolling_windows", "feature_version")
...
def load_model_state():
    ...
    global last_loaded_at, status_phase, status_message, last_status_error, model_meta_warning_fields
    try:
        ...
        if warning_fields:
            print(
                "[predictor-long-lstm] loaded saved model with compatibility warning:"
...
def _save_lstm_analytics(X_scaled: Optional[np.ndarray] = None, y_scaled: Optional[np.ndarray] = None) -> None:
    if model is None or torch is None:
        return
    try:
        os.makedirs(MODEL_DIR, exist_ok=True)
        feature_dim = int(model_feature_dim or len(LONG_TERM_FEATURE_NAMES))
        feature_names = _feature_names()[:feature_dim]
        analytics = {
            "model": "LSTM",
        ...
        if X_scaled is not None and y_scaled is not None:
            X_eval = np.asarray(X_scaled, dtype=float)
            y_eval = np.asarray(y_scaled, dtype=float).reshape(-1)
            if X_eval.ndim == 3 and y_eval.size == X_eval.shape[0] and X_eval.shape[2] >= feature_dim and len(X_eval) >= 8:
                sample_count = min(len(X_eval), 256)
                X_eval = X_eval[-sample_count:]
                y_eval = y_eval[-sample_count:]
                baseline = _predict_scaled_batch(X_eval).reshape(-1)
                baseline_mse = float(np.mean((baseline - y_eval) ** 2))
                rng = np.random.default_rng(42)
                importances = np.zeros(feature_dim, dtype=float)
                for feature_idx in range(feature_dim):
                    X_permuted = np.array(X_eval, copy=True)
                    perm = rng.permutation(sample_count)
                    X_permuted[:, :, feature_idx] = X_eval[perm, :, feature_idx]
                    permuted = _predict_scaled_batch(X_permuted).reshape(-1)
                    permuted_mse = float(np.mean((permuted - y_eval) ** 2))
                    importances[feature_idx] = max(0.0, permuted_mse - baseline_mse)
                ...
                analytics["all_features"] = [
                    {"feature": feature_names[idx], "importance": float(normalized[idx])}
                ...
                corr_idx = order[: min(12, feature_dim)]
                last_step_features = X_eval[:, -1, corr_idx]
                corr_input = np.column_stack([last_step_features, y_eval])
        ...
        with open(LSTM_ANALYTICS_PATH, "w", encoding="utf-8") as handle:
            json.dump(analytics, handle, indent=2)
    except Exception as exc:
        print(f"[predictor-long-lstm] failed to save analytics: {exc}")
...
def _load_bootstrap_payloads() -> list[dict]:
    ...
    if not log_dir.exists():
        ...
        print(f"[predictor-long-lstm] bootstrap skipped: log dir not found: {log_dir}")
        return []
    ...
    if not files:
        ...
        print(f"[predictor-long-lstm] bootstrap skipped: no telemetry logs for {PLANT_IDENTIFIER} in {log_dir}")
        return []
    ...
    for path in files:
        try:
            ...
            print(f"[predictor-long-lstm] bootstrap skipped unreadable log {path}: {exc}")
...
def _status_payload(extra: Optional[dict] = None) -> dict:
    payload = {
        ...
        "meta_path": LSTM_META_PATH,
        "analytics_path": LSTM_ANALYTICS_PATH,
...
def _publish_status(client=None, extra: Optional[dict] = None) -> None:
    if client is None:
        return
    try:
        client.publish(TOPIC_PREDICTION_LONG_LSTM_STATUS_NAME, json.dumps(_status_payload(extra)), qos=1, retain=True)
    except Exception as exc:
        print(f"[predictor-long-lstm] failed to publish status: {exc}")


def _train_model_from_sequences(client=None) -> bool:
    global model_ready, model, model_feature_dim, last_train_index, warned_eval_without_model, last_trained_at
    global status_phase, status_message, last_status_error
    if len(train_X) < MIN_TRAIN_SAMPLE_COUNT:
        return False
...
def bootstrap_from_history(client=None) -> int:
    ...
    _train_model_from_sequences(client=client)
    bootstrap_active = False
    last_bootstrap_completed_at = time.time()
    ...
    print(
        f"[predictor-long-lstm] bootstrap replayed {len(payloads)} historical telemetry samples "
        f"from {LOGGER_OUTPUT_DIR}; train_samples={len(train_X)} model_ready={model_ready}"
    )
    return len(payloads)
...
def _start_bootstrap_thread(client=None) -> bool:
    ...
    def _worker():
        ...
        try:
            ...
            print(f"[predictor-long-lstm] bootstrap failed: {exc}")
            _publish_status(client)

    bootstrap_thread = threading.Thread(target=_worker, name=f"{PLANT_IDENTIFIER}-lstm-bootstrap", daemon=True)
...
def build_step_feature():
    ...
    if len(features) != len(LONG_TERM_FEATURE_NAMES):
        raise RuntimeError(f"step_feature length mismatch: {len(features)} != {len(LONG_TERM_FEATURE_NAMES)}")
    return features
...
def _step_feature_with_load_overrides(load_metrics: Dict[str, float], power_override: Optional[float] = None):
    ...
    if len(features) != len(LONG_TERM_FEATURE_NAMES):
        raise RuntimeError(f"step_feature override length mismatch: {len(features)} != {len(LONG_TERM_FEATURE_NAMES)}")
...
def build_mpc_process_plan_suggestions(
    ...
    for mode, dev in candidates:
        ...
        for duty in duty_steps:
            ...
            predicted_window_energy_wh = max(0.0, float(y_scaler.inverse_transform(y_hat_s.reshape(-1, 1))[0][0]))
...
class PowerLSTM(nn.Module if nn is not None else object):
    def __init__(self, feature_dim: int):
        if nn is None:
            raise RuntimeError("torch backend unavailable")
        super().__init__()
        self.lstm1 = nn.LSTM(input_size=feature_dim, hidden_size=48, batch_first=True)
...
def _fit_scaled_batch(Xs: np.ndarray, ys: np.ndarray):
    if model is None or torch is None:
        raise RuntimeError("model backend unavailable")
    sample_count = int(Xs.shape[0]) if Xs.ndim >= 1 else 0
    val_count = max(0, int(sample_count * 0.1)) if sample_count >= 64 else 0
    if val_count >= sample_count:
        val_count = max(0, sample_count - 1)

    if val_count > 0:
        train_Xs = Xs[:-val_count]
        train_ys = ys[:-val_count]
        val_Xs = Xs[-val_count:]
        val_ys = ys[-val_count:]
    else:
        train_Xs = Xs
    ...
    dataset = TensorDataset(
        torch.as_tensor(train_Xs, dtype=torch.float32),
        torch.as_tensor(train_ys, dtype=torch.float32),
    )
    loader = DataLoader(dataset, batch_size=min(BATCH_SIZE, max(1, len(dataset))), shuffle=True)
    ...
    for _ in range(max(1, EPOCHS)):
        ...
        if val_Xs is not None and val_ys is not None:
            model.eval()
            with torch.no_grad():
                val_pred = model(torch.as_tensor(val_Xs, dtype=torch.float32))
                val_loss = float(criterion(val_pred, torch.as_tensor(val_ys, dtype=torch.float32)).item())
...
def apply_config(cfg: dict):
    ...
    global model, model_ready, model_feature_dim, PREDICTOR_MODE, warned_eval_without_model, model_meta_warning_fields
    ...
    if "SEQUENCE_LENGTH" in cfg:
        next_sequence_length = max(10, int(cfg["SEQUENCE_LENGTH"]))
        architecture_changed = architecture_changed or next_sequence_length != SEQUENCE_LENGTH
    ...
    if cfg.get("RESET_MODEL", False) or architecture_changed:
        model = None
        model_ready = False
        model_feature_dim = 0
        warned_eval_without_model = False
...
def _process_telemetry_payload(data, client=None, publish: bool = True, bootstrap: bool = False):
    ...
    if _training_enabled():
        ...
        if len(step_feature_buffer) >= SEQUENCE_LENGTH:
            ...
            pending_supervised.append((target_idx, seq, current_total_energy_wh))

        # Resolve any due targets with current observed power.
        while pending_supervised and pending_supervised[0][0] <= sample_index:
            _, seq_X, start_total_energy_wh = pending_supervised.popleft()
            train_X.append(seq_X)
            train_y.append(max(0.0, current_total_energy_wh - float(start_total_energy_wh)))

        # Periodic batch retraining.
        if not bootstrap and len(train_X) >= MIN_TRAIN_SAMPLE_COUNT and (sample_index - last_train_index) >= TRAIN_INTERVAL_STEPS:
            _train_model_from_sequences(client=client)
