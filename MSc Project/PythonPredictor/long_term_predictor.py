...
FORECAST_HORIZON_SEC = 1800             # 30 minutes
TELEMETRY_SAMPLE_SEC = 5.0               # simulator default is every 5 seconds
...
PREDICTOR_MODE = os.getenv("LONG_TERM_PREDICTOR_MODE", "OFFLINE").strip().upper()
...
# Feature engineering
LAG_STEPS = [1, 5, 15, 60, 300]      # lag steps
...
def feature_vector():
    ...
    # current power
    features.append(p_arr[-1])

    # lags
    for lag in LAG_STEPS:
        if len(p_arr) > lag:
            features.append(p_arr[-lag])
        else:
            features.append(p_arr[0])
    ...
    features.append(sin_h)
    features.append(cos_h)
...
def build_mpc_process_plan_suggestions(
    ...
    for mode, dev in candidates:
        ...
        for duty in duty_steps:
            ...
            if feat is None:
                continue
            predicted_power = float(model.predict([feat])[0])
...
def apply_config(cfg: dict):
    ...
    global PREDICTOR_MODE, model_ready, model, warned_eval_without_model
...
def on_message(client, userdata, msg):
    ...
    if _training_enabled():
        ...
        pending_feature_buffer.append(features)
        if len(pending_feature_buffer) > FORECAST_HORIZON_STEPS:
            feature_buffer.append(pending_feature_buffer.popleft())
            target_buffer.append(total_power_w)

        # Train periodically.
        if len(feature_buffer) >= MIN_TRAIN_SAMPLE_COUNT and (len(feature_buffer) % TRAIN_INTERVAL_STEPS == 0):
    ...
    predicted_power_w = float(model.predict([features])[0])
